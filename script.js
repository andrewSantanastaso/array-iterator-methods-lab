...
firstLast = people.map((person) => {
    let rearrange = person.split(', ')
    return `${rearrange[1]} ${rearrange[0]}`
})
...
totalYearsLived = inventors.reduce((runningTotal, inventor, index) => {
...
travelMethodCounts = travelMethods.reduce((acc, method, index) => {
    if (acc[method]) {
        acc[method]++
    }
    else {
